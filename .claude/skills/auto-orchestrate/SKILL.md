...
> **핵심**: 각 태스크를 독립 tmux 패널의 Claude CLI 프로세스로 실행. 완전한 프로세스 격리.
...
│  ├── Claude Code 내부 서브에이전트                               │
│  ├── 서브에이전트 중첩 불가 (1단계만)                            │
│  └── 메인 컨텍스트 프로세스 공유                                 │
│                                                                 │
│  tmux 모드 (--tmux):                                            │
│  ├── 완전 독립 OS 프로세스                                       │
│  ├── 각 에이전트 자체 컨텍스트 윈도우 보유                       │
│  ├── 중첩 제한 없음                                              │
...
```
1단계: TASKS.md 분석 → 의존성 기반 병렬 그룹 생성
    ↓
2단계: tmux 세션 생성 (tmux new-session -d -s vibe)
    ↓
3단계: 그룹별 순차 실행 (그룹 내 병렬)
  ├── 각 태스크마다 tmux split-window
  ├── claude --dangerously-skip-permissions -p '...' 실행
  └── 완료 신호: /tmp/task-{ID}.done
    ↓
4단계: 완료 모니터링 (while 루프로 .done 파일 체크)
    ↓
5단계: 결과 취합 (/tmp/task-{ID}-result.md 읽기)
    ↓
6단계: Phase 병합 → 다음 그룹 진행
```
...
```bash
# tmux 병렬 모드 실행
/auto-orchestrate --tmux

# 최대 4개 동시 실행
/auto-orchestrate --tmux --parallel 4

# 특정 Phase만
/auto-orchestrate --tmux --phase 2

# 중단 후 재개
/auto-orchestrate --tmux --resume
```

> 상세 내용: `references/tmux-parallel.md`
...
| `--tmux` | **tmux 병렬 모드** (독립 프로세스 기반 병렬 실행) |
| `--tmux --parallel N` | tmux 최대 동시 패널 수 (기본: 3) |
...
| tmux 병렬 실행 | references/tmux-parallel.md | `--tmux` 모드 선택 시 |
