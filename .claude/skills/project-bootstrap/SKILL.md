...
> **프론트엔드=Next.js + DB=Supabase 조합이 감지되면, 4단계 진행을 중단하고 `/project-bootstrap-supabase` 스킬로 자동 위임한다.**

### 감지 시점

2단계에서 프론트엔드와 DB 선택이 완료된 직후 판단:

| 프론트엔드 | DB | 동작 |
|-----------|-----|------|
| Next.js | Supabase | `/project-bootstrap-supabase` 스킬로 위임 |
| Next.js | 그 외 | 기존 흐름 계속 |
| 그 외 | Supabase | 기존 흐름 계속 (Supabase를 외부 DB로 사용) |
| 그 외 | 그 외 | 기존 흐름 계속 |

### 위임 방법

```
Next.js + Supabase 조합이 감지되었습니다.
전용 스킬(/project-bootstrap-supabase)로 전환합니다.
Supabase 로컬 스택, SSR 보일러플레이트, CI/CD 파이프라인까지 자동 셋업됩니다.
```

→ `Skill({ skill: "project-bootstrap-supabase" })` 호출
...
| Supabase | - | 클라우드 BaaS (Next.js 선택 시 전용 스킬로 위임) |
| Firebase | - | 클라우드 NoSQL |
